...
function buildKeywords(jobDesc, shiftType) {
  const shift = shiftType && shiftType !== "Any" ? ` ${shiftType.toLowerCase()}` : "";
  const lines = jobDesc.split(/[\n.]/);
  const first = lines[0].replace(/[^a-zA-Z0-9 ]/g, " ").trim().split(/\s+/).slice(0, 5).join(" ");
  const second = (lines.find((l, i) => i > 0 && l.trim().length > 10) || "").replace(/[^a-zA-Z0-9 ]/g, " ").trim().split(/\s+/).slice(0, 4).join(" ");
  return [...new Set([first + shift, second ? second + shift : null].filter(Boolean))];
}
...
function Dashboard({ client, onEdit, onBack, onSettings, onClientUpdate }) {
  ...
  const runSearch = async () => {
    setPhase("searching"); setStatusMsg("Building search terms…");
    setErrMsg(""); setJobs([]); setScoredCount(0); setBulkDone(false); setTailored({});
    try {
      const kws = buildKeywords(client.jobDesc, client.shiftType || "Any");
      const allJobs = []; const seen = new Set();

      for (let i = 0; i < kws.length; i++) {
        const kw = kws[i];
        setStatusMsg(`Searching Reed.co.uk for "${kw}"… (${i + 1}/${kws.length})`);
      ...
      if (allJobs.length === 0) {
        ...
        setErrMsg("No jobs found on Reed for this search. Try editing the job description or use Manual Paste.");
        return;
      }
